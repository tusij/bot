...
func main() {
	var bizConfig config.BizConfig
	err := utils.LoadConfig("config.yaml", &bizConfig)
	if err != nil {
		log.Fatalf("load biz config error %v", err)
	}
	openAPI = &openapi.OpenAPImpl{}
	openAPI = openAPI.Setup(bizConfig.OpenAPIConfig)
	...
	weatherService = service.NewJuHeWeatherService(bizConfig.WeatherServiceConfig)
	...
	if err = manager.Start(wsInfo, bizConfig.OpenAPIConfig.Token, &intent); err != nil {
...
func atMessageHandle(weatherService service.WeatherService) event.ATMessageEventHandler {
	return func(event *dto.WSPayload, data *dto.Message) error {
		...
		var message *dto.MessageToCreate = &dto.MessageToCreate{
			Content: "我是享享 目前只支持天气查询功能 请输入：天气 城市命令查询天气\n" +
				"例如:天气 深圳",
